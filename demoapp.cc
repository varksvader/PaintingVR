...
void DemoApp::DrawEye(gvr::Eye which_eye, const gvr::Mat4f& eye_view_matrix,
    ...
    else if(!painting_ &&controller_state_.GetButtonState(gvr::kControllerButtonApp)&&count<=100){
        count++;
    }
    else if (!painting_ &&
        controller_state_.GetButtonState(gvr::kControllerButtonApp)&& count>100) {
        committed_vbos_.clear();
    }
    else if(!painting_ && controller_state_.GetButtonUp(gvr::kControllerButtonApp)){
        count=0;
    }
